...
def fillhole(input_image):
    ...
    input_image = np.reshape(input_image,(h,w))
    ...
    return np.expand_dims(img_out, axis=2)
...
def MergeMaskedFace (predictor_func, predictor_input_shape,
    ...
    img_face_landmarks = np.array(img_face_landmarks)
    ...
    # cv2.imwrite("Dst_face_bgr.png",dst_face_bgr)
    ...
    # print("REsize good")
    # print(vars(xseg_256_extract_func))
    predicted = predictor_func.func (predictor_input_bgr)
    ...
    if cfg['mask_mode'] == 0: #full
        ...
        if cfg['mask_mode'] == 6 or cfg['mask_mode'] == 8 or cfg['mask_mode'] == 9:
            ...
            prd_face_xseg_mask  = fillhole(prd_face_xseg_mask)
    ...
    #del img_face_landmarks, prd_face_bgr_enhanced,prd_face_xseg_mask,dst_face_xseg_mask
    return out_img, out_merging_mask_a


def MergeMaskedFace_assume (predictor_func, predictor_input_shape,
                     face_enhancer_func,
                     xseg_256_extract_func,
                     cfg,  img_bgr_uint8, img_bgr,img_bgr_ori, img_face_landmarks,raw_merge = False):
    img_size = img_bgr.shape[1], img_bgr.shape[0]
    img_face_landmarks = np.array(img_face_landmarks)
    img_face_mask_a = LandmarksProcessor.get_image_hull_mask (img_bgr.shape, img_face_landmarks)
    ...
    dst_face_bgr      = np.clip(dst_face_bgr, 0, 1)
    #
    # dst_face_bgr_ori      = cv2.warpAffine( img_bgr_ori, face_mat, (output_size, output_size), flags=cv2.INTER_CUBIC )
    # dst_face_bgr_ori      = np.clip(dst_face_bgr_ori, 0, 1)
    ...
    if raw_merge:
        prd_face_bgr          = np.clip (predictor_input_bgr, 0, 1.0)
        # prd_face_mask_a_0     = np.clip (predictor_input_bgr[1], 0, 1.0)
        # prd_face_dst_mask_a_0 = np.clip (predictor_input_bgr[2], 0, 1.0)
    else:
        predicted = predictor_func.func (predictor_input_bgr)
        prd_face_bgr          = np.clip (predicted[0], 0, 1.0)
        prd_face_mask_a_0     = np.clip (predicted[1], 0, 1.0)
    ...
    if cfg['mask_mode'] == 0: #full
        ...
        if cfg['mask_mode'] >= 7 and cfg['mask_mode'] <= 9:
            # obtain XSeg-dst
            xseg_mat            = LandmarksProcessor.get_transform_mat (img_face_landmarks, xseg_input_size, face_type=cfg['face_type'])
            dst_face_xseg_bgr   = cv2.warpAffine(img_bgr_ori, xseg_mat, (xseg_input_size,)*2, flags=cv2.INTER_CUBIC )
    ...
    if cfg['mode'] == 'original':
        ...
        if maxregion.size != 0:
            ...
            if min(lenx,leny) >= 4:
                ...
                out_img = img_bgr_ori*(1-img_face_mask_a) + (out_img*img_face_mask_a)
...
def MergeMasked(predictor_func,
    ...
    if remerge:
        ...
        cfg["mode"] = "overlay"
        cfg['output_face_scale'] = -1

    # print(cfg)
    img_bgr_uint8 = images#data['faceimage']

    img_bgr_uint8 = imagelib.normalize_channels (img_bgr_uint8, 3)
    img_bgr = img_bgr_uint8.astype(np.float32) / 255.0

    img_bgr_uint8_ori = image_ori#data['faceimage']

    img_bgr_uint8_ori = imagelib.normalize_channels (img_bgr_uint8_ori, 3)
    img_bgr_ori = img_bgr_uint8_ori.astype(np.float32) / 255.0

    outs = []
    # for face_num, img_landmarks in enumerate( data['image_landmarks'] ):
    img_landmarks = data['image_landmarks']
    out_img, out_img_merging_mask = MergeMaskedFace_assume (predictor_func, predictor_input_shape, face_enhancer_func, xseg_256_extract_func, cfg,  img_bgr_uint8, img_bgr, img_bgr_ori, img_landmarks,raw_merge=skip_merge)
    # out_img, out_img_merging_mask = MergeMaskedFace (predictor_func, predictor_input_shape, face_enhancer_func, xseg_256_extract_func, cfg,  img_bgr_uint8, img_bgr, img_landmarks)

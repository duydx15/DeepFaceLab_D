...
class ExtractSubprocessor(Subprocessor):
    ...
    class Cli(Subprocessor.Cli):
        ...
        def rects_stage(data,
            ...
            if min(h,w) < 128:
                ...
                for rot in ([0, 90, 270, 180]):
                    ...
                    print("Size image S3FD:",rotated_image.shape)
                    if len(rects) != 0:
        ...
        def landmarks_stage(data,
            ...
            h, w, ch = image.shape
            # print("data['rects_rotation']",data['rects_rotation'])
        ...
        def final_stage(data,
            ...
            data['face_image_landmarks'] = face_image_landmarks.tolist()
            data['faces_detected'] = face_idx
            data['image_landmarks'] = image_landmarks.tolist()
        ...
        @staticmethod
        def landmarks_stage_mediapipe(data,
                            image,
                            landmarks_extractor_mediapipe,
                            rects_extractor,
                            ):
            ...
            results = landmarks_extractor_mediapipe.process(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
            # print(vars(results))
            listpoint2 = []
            landmarks_mp = results.multi_face_landmarks[0]
            # print(vars(landmarks_mp))
            for i in range(len(landmark_points_68)):
                idx = landmark_points_68[i]
                x = landmarks_mp.landmark[idx].x
                y = landmarks_mp.landmark[idx].y
                listpoint2.append([x,y])

            data['landmarks'] = np.array(listpoint2, np.int32)
            # print("Mediapipe",data['landmarks'])
            if data['rects_rotation'] != 0:
                for i, (rect, lmrks) in enumerate(zip(data['rects'], data['landmarks'])):
                    new_rect, new_lmrks = rect, lmrks
                    (l,t,r,b) = rect
...
def main_extract(face_type=None,
    ...
    config_extract['face_type'] = face_type
    config_extract['jpeg_quality'] = jpeg_quality
    config_extract['image_size'] = image_size
    config_extract['max_faces_from_image'] = max_faces_from_image
